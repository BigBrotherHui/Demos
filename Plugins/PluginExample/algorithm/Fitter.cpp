...
vector<double> Fitter::uniformParametrization(const vector<Eigen::Vector3d>& dataPoints) const {
...
vector<double> Fitter::chordalParametrization(const std::vector<Eigen::Vector3d>& dataPoints) const {
	int size = static_cast<int>(dataPoints.size());
	double accumulation = 0, total = 0;
	for (int i = 0; i < size - 1; ++i) {
		total += (dataPoints[i + 1] - dataPoints[i]).norm();
	}
	if (equal(total, 0)) {
	...
	for (int i = 1; i < size - 1; ++i) {
		accumulation += (dataPoints[i + 1] - dataPoints[i]).norm();
...
vector<double> Fitter::centripetalParametrization(const vector<Eigen::Vector3d>& dataPoints) const {
	int size = static_cast<int>(dataPoints.size());
	double accumulation = 0, total = 0;
	for (int i = 0; i < size - 1; ++i) {
		total += sqrt((dataPoints[i + 1] - dataPoints[i]).norm());
	}
	if (equal(total, 0)) {
		return uniformParametrization(dataPoints);
	}
	...
	ret[0] = 0;
	for (int i = 1; i < size - 1; ++i) {
		accumulation += sqrt((dataPoints[i + 1] - dataPoints[i]).norm());
		ret[i] = accumulation / total;
	}
	ret[size - 1] = 1;
	return ret;
...
vector<double> Fitter::curveParametrization(const std::vector<Eigen::Vector3d>& dataPoints) const {
	if (pType == 0) {
		return uniformParametrization(dataPoints);
	}
	if (pType == 1) {
...
vector<vector<double>> Fitter::surfaceParametrization(const vector<vector<Eigen::Vector3d>>& dataPoints) const {
	...
	for (int j = 0; j <= n; ++j) {
		vector<Eigen::Vector3d> columnData(m + 1);
		for (int i = 0; i <= m; ++i) {
	...
	for (int i = 0; i <= m; ++i) {
		vector<Eigen::Vector3d> rowData(n + 1);
		for (int j = 0; j <= n; ++j) {
...
std::vector<Eigen::Vector3d> Fitter::interpolateControlPoints(const BSpline& bsp,
                                                   const vector<Eigen::Vector3d>& dataPoints,
...
BSplineSurface* Fitter::interpolateSurface(int p,
                                           int q,
                                           const std::vector<std::vector<Eigen::Vector3d>>& dataPoints,
	...
	vector<vector<Eigen::Vector3d>> Q(m + 1, vector<Eigen::Vector3d>(n + 1));
	for (int d = 0; d <= n; ++d) {
		vector<Eigen::Vector3d> columnDataPoints(m + 1);
		for (int i = 0; i <= m; ++i) {
			columnDataPoints[i] = dataPoints[i][d];
		}
		vector<Eigen::Vector3d> intermediate = interpolateControlPoints(bs->ubsp, columnDataPoints, ts[0]);
		for (int i = 0; i <= m; ++i) {
...
std::vector<Eigen::Vector3d> Fitter::approximateControlPoints(const BSpline& bsp,
                                                   int h,
                                                   const std::vector<Eigen::Vector3d>& dataPoints,
                                                   const std::vector<double>& ts) const {
	int n = static_cast<int>(dataPoints.size()) - 1;
	...
	vector<Eigen::Vector3d> controlPoints(h + 1);
	controlPoints[0] = dataPoints[0];
	for (int i = 1; i < h; ++i) {
		auto p = P.row(i - 1);
		controlPoints[i] = Eigen::Vector3d(p(0), p(1), p(2));
	}
...
BSpline* Fitter::approximateCurve(int p,
                                  int h,
                                  const std::vector<Eigen::Vector3d>& dataPoints,
                                  std::vector<Eigen::Vector3d>& controlPoints) const {
	std::vector<double> ts = curveParametrization(dataPoints);
...
BSplineSurface* Fitter::approximateSurface(int p,
                                           ...
                                           int f,
                                           const std::vector<std::vector<Eigen::Vector3d>>& dataPoints,
                                           std::vector<std::vector<Eigen::Vector3d>>& controlPoints) const {
	vector<vector<double>> ts = surfaceParametrization(dataPoints);
	...
	int m = static_cast<int>(dataPoints.size()) - 1, n = static_cast<int>(dataPoints[0].size()) - 1;
	vector<vector<Eigen::Vector3d>> Q(e + 1, vector<Eigen::Vector3d>(n + 1));
	for (int d = 0; d <= n; ++d) {
		vector<Eigen::Vector3d> columnDataPoints(m + 1);
		for (int i = 0; i <= m; ++i) {
			columnDataPoints[i] = dataPoints[i][d];
		}
		vector<Eigen::Vector3d> intermediate = approximateControlPoints(bs->ubsp, e, columnDataPoints, ts[0]);
		for (int i = 0; i <= e; ++i) {
			Q[i][d] = intermediate[i];
		}
	}
