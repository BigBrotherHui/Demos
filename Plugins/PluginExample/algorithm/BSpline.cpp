...
Eigen::Vector3d BSpline::operator()(const std::vector<Eigen::Vector3d>& controlPoints, double u) const {
...
Eigen::Vector3d BSplineSurface::operator()(const vector<vector<Eigen::Vector3d>>& controlPoints, double u, double v) const {
	int m = static_cast<int>(controlPoints.size()) - 1, n = static_cast<int>(controlPoints[0].size()) - 1;
	vector<Eigen::Vector3d> intermediate(m + 1);
	for (int i = 0; i <= m; ++i) {
		vector<Eigen::Vector3d> rowControlPoints(n + 1);
		for (int j = 0; j <= n; ++j) {
