...
const mitk::Surface *mitk::CustomSurfaceVtkMapper3D::GetInput()
{
  return static_cast<const mitk::Surface *>(GetDataNode()->GetData());
}

mitk::CustomSurfaceVtkMapper3D::CustomSurfaceVtkMapper3D()
{
    luTable = vtkSmartPointer<vtkLookupTable>::New();
    luTable->SetNumberOfTableValues(11);
    luTable->SetTableRange(0, 11);
    luTable->SetTableValue(0, 0, 1, 1, 1);
    luTable->SetTableValue(1, 1, 1, 1, 1);
    luTable->SetTableValue(2, 0, 1, 0, 1);
    luTable->SetTableValue(3, 1, 1, 0, 1);
    luTable->SetTableValue(4, 1, 1, 1, 1);
    luTable->SetTableValue(5, 1, 0, 0, 1);
    luTable->SetTableValue(6, 1, 0, 1, 1);
    luTable->SetTableValue(7, 1, 1, 0, 1);
    luTable->SetTableValue(8, 0, 1, 1, 1);
    luTable->SetTableValue(9, 1, 1, 0, 1);
    luTable->SetTableValue(10, 1.0, 0, 0, 1);
    luTable->Build();
...
void mitk::CustomSurfaceVtkMapper3D::GenerateDataForRenderer(mitk::BaseRenderer *renderer)
{
...
//        ls->m_VtkPolyDataMapper->SetScalarVisibility(1);
        ls->m_VtkPolyDataMapper->SetLookupTable(luTable);
//        ls->m_VtkPolyDataMapper->SetScalarModeToUsePointData();
        ls->m_VtkPolyDataMapper->SetScalarRange(0, luTable->GetNumberOfTableValues() - 1);
        ls->m_VtkPolyDataMapper->Update();
...
void mitk::CustomSurfaceVtkMapper3D::ResetMapper(BaseRenderer *renderer)
{
  LocalStorage *ls = m_LSH.GetLocalStorage(renderer);
  ls->m_Actor->VisibilityOff();
}

void mitk::CustomSurfaceVtkMapper3D::ApplyMitkPropertiesToVtkProperty(mitk::DataNode *node,
...
void mitk::CustomSurfaceVtkMapper3D::ApplyAllProperties(mitk::BaseRenderer *renderer, vtkActor * /*actor*/)
{
  LocalStorage *ls = m_LSH.GetLocalStorage(renderer);
...
vtkProp *mitk::CustomSurfaceVtkMapper3D::GetVtkProp(mitk::BaseRenderer *renderer)
{

//    qDebug() << "mitk::SurfaceVtkMapper3D::GetVtkProp ... ...";
  LocalStorage *ls = m_LSH.GetLocalStorage(renderer);

  return ls->m_Actor;
}

void mitk::CustomSurfaceVtkMapper3D::CheckForClippingProperty(mitk::BaseRenderer *renderer,
...
void mitk::CustomSurfaceVtkMapper3D::SetDefaultPropertiesForVtkProperty(mitk::DataNode *node,
                                                                  mitk::BaseRenderer *renderer,
                                                                  bool overwrite)
{
  // Shading
...
void mitk::CustomSurfaceVtkMapper3D::SetDefaultProperties(mitk::DataNode *node, mitk::BaseRenderer *renderer,
                                                          bool overwrite)
{
  node->AddProperty("color", mitk::ColorProperty::New(1.0f, 1.0f, 1.0f), renderer, overwrite);
  node->AddProperty("opacity", mitk::FloatProperty::New(1.0), renderer, overwrite);

  mitk::CustomSurfaceVtkMapper3D::SetDefaultPropertiesForVtkProperty(node, renderer, overwrite);  // Shading
