...
void GACDriver::sigmoid_filtering(FloatImageType::Pointer inputimage) {
    ...
    {
        std::cout << "sigmoid error" << e.what() << std::endl;
    }
    ...
    {
        std::cout << "caster error" << e.what() << std::endl;
    }
...
void GACDriver::initial_gacfilter() {
    //geodesicActiveContour->SetInitialImage(m_gacparameters->m_fastmarchingImage);
    ...
    {
        std::cout << "error geodesicActiveContour:" << e.what() << std::endl;
    }
}
...
void GACDriver::create_bubble_image(){
    ...
    auto spacing=m_imagedata->m_frontground_image->GetSpacing();
    auto origin = m_imagedata->m_frontground_image->GetOrigin();
    double sp[3];
    double or [3];
    for(int i=0;i<3;++i)
    {
    ...
    m_imagedata->m_frontground_image->SetOrigin(or );
    m_imagedata->m_frontground_image->SetSpacing(sp);
    m_mainwindow->displayImage();
}