...
public:
    ...
    virtual void Execute(vtkObject *, unsigned long vtkNotUsed(event), void *)
    {
        ...
        z=renderer->GetZ(x,y);
        this->Picker->Pick(x, y, z, renderer);
        ...
        double ijk[3];
        this->Viewer->GetInput()->TransformPhysicalPointToContinuousIndex(pos, ijk);

        int axis = this->Viewer->GetSliceOrientation();
        switch (axis)
        {
            case vtkImageViewer2::SLICE_ORIENTATION_XZ:
                image_coordinate[0] = vtkMath::Round(ijk[0]);
                image_coordinate[1] = this->Viewer->GetSlice();
                image_coordinate[2] = vtkMath::Round(ijk[2]);
                break;
            case vtkImageViewer2::SLICE_ORIENTATION_YZ:
                image_coordinate[0] = this->Viewer->GetSlice();
                image_coordinate[1] = vtkMath::Round(ijk[0]);
                image_coordinate[2] = vtkMath::Round(ijk[1]);
                break;
            default:  // vtkImageViewer2::SLICE_ORIENTATION_XY
                image_coordinate[0] = vtkMath::Round(ijk[0]);
                image_coordinate[1] = vtkMath::Round(ijk[1]);
