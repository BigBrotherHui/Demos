    ...
    ui(new Ui::MainWindow)
{
    ...
    m_Connections->Connect(ui->qvtkopenglWidget->renderWindow()->GetInteractor(),
                           vtkCommand::MouseMoveEvent,
                           this,
                           SLOT(updateCoords(vtkObject*)));

    // qApp就是現在的application，用來檢測在螢幕間切換的pixel問題
    connect(qApp, SIGNAL(primaryScreenChanged(QScreen*)), this, SLOT(screenChanged()));
...
void MainWindow::show_originThumbnail() {
    ...
    int rows = size[0];
    int cols = size[1];
    uchar** imagio = (uchar**)malloc(cols * sizeof(uchar*));
    if (imagio == NULL) {
        printf("内存分配失败\n");
        return ;
    }
    ...
    for (int i = 0; i < cols; i++) {
        imagio[i] = (uchar*)malloc(rows * sizeof(uchar));
        if (imagio[i] == NULL) {
            printf("内存分配失败\n");
...
void MainWindow::show_edgeThumbnail() {
    ...
    int rows = size[0];
    int cols = size[1];
    unsigned char** imagio = (unsigned char**)malloc(cols * sizeof(unsigned char*));
    if (imagio == NULL) {
        printf("内存分配失败\n");
        return ;
    }
    ...
    for (int i = 0; i < cols; i++) {
        imagio[i] = (unsigned char*)malloc(rows * sizeof(unsigned char));
        if (imagio[i] == NULL) {
            printf("内存分配失败\n");
    ...
    QImage image((uchar*)imagio, rows, cols, rows, QImage::Format_Grayscale16);
...
void MainWindow::picker_setup() {
    ...
    ui->qvtkopenglWidget->GetInteractor()->GetInteractorStyle()->AddObserver(vtkCommand::MouseMoveEvent, m_callback);
...
void MainWindow::displayImage() {
    ...
    if (!m_imagedata->m_background_image){
        ui->qvtkopenglWidget->renderWindow()->ClearInRenderStatus();
        ...
        m_ImageViewer->SetRenderWindow(ui->qvtkopenglWidget->renderWindow());

        m_Renderer->ResetCamera();
        ui->qvtkopenglWidget->renderWindow()->Render();
    }
    else {
//        m_BackgroundRenderer->Clear();
        ...
        ui->qvtkopenglWidget->renderWindow()->SetNumberOfLayers(2);
        ui->qvtkopenglWidget->renderWindow()->AddRenderer(m_BackgroundRenderer);
        ui->qvtkopenglWidget->renderWindow()->AddRenderer(m_Renderer);
        ui->qvtkopenglWidget->renderWindow()->Render();
    }
}
