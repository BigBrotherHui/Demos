...
//    ui->comboBox_vetebra->setCheckExclusive(true);
    ...
    styleImageFront=vtkSmartPointer<InteractorStyleImage>::New();
    styleImageSide=vtkSmartPointer<InteractorStyleImage>::New();
    ...
    interatorFront->SetInteractorStyle(styleImageFront);
    ...
    interatorSide->SetInteractorStyle(styleImageSide);
...
void RegisterFineAdjustDialog::showImages(vtkImageData *pMergedFront,vtkImageData *pMergedSide)
{
    ...
    bool isPickEnable=ui->checkBox_pickPoint->isChecked();
    styleImageFront->setImageData(pMergedFront);
    //styleImageSide->setImageData(pMergedSide);
    styleImageFront->setPickEnable(isPickEnable);
    //styleImageSide->setPickEnable(isPickEnable);

    styleImageFront->markPixel();
    //styleImageSide->markPixel();
...
void RegisterFineAdjustDialog::addMenu(QVTKOpenGLNativeWidget *pTarget)
{
    ...
    L5->setMenu(menuL5);
    //开始添加Action
    QAction *clearAction=new QAction("清除",pTarget);
    connect(clearAction,&QAction::triggered,this,[&]
            {
                if(mJsonObject.find("PickedPosArray")!=mJsonObject.end())
                {
                    QJsonArray ary=mJsonObject["PickedPosArray"].toArray();
                    for(int i=0;i<ary.size();i++)
                    {
                        QString key=ary.at(i).toString();
                        if(mJsonObject.end()!=mJsonObject.find(key))
                        {
                            mJsonObject.remove(key);
                        }
                    }
                    mJsonObject.remove("PickedPosArray");
                }
                QFile file(mRegistrationFile);
                if(!file.open(QIODevice::ReadWrite))
                    return;
                QJsonDocument doc=QJsonDocument::fromJson(file.readAll());
                doc.setObject(mJsonObject);
                QByteArray bt=doc.toJson();
                file.resize(0);
                file.write(bt);
                file.close();
            });
    pTarget->addAction(clearAction);
    ...
    QList<QMenu *> menus{menuL1,menuL2,menuL3,menuL4,menuL5};
    for(auto &menu : menus )
    {
        connect(menu,&QMenu::triggered,this,[&](QAction *action)
                {
                    QString text=static_cast<QAction *>(action->parent())->text()+action->text();
                    QWidget *wg=static_cast<QWidget *>(static_cast<QAction *>(action->parent())->menu()->parent());
                    double pickedpt[3]{0};
                    vtkImageData *imageData{nullptr};
                    if(wg==wgMergedFront)
                    {
                        styleImageFront->lastPickedPoint(pickedpt);
                        imageData=styleImageFront->imageData();
                    }
                    else
                    {
                        styleImageSide->lastPickedPoint(pickedpt);
                        imageData=styleImageSide->imageData();
                    }
                    if(!imageData)
                        return;
                    QFile file(mRegistrationFile);
                    ...
                    if(mJsonObject.find("PickedPosArray")==mJsonObject.end())
                    {
                        mJsonObject.insert("PickedPosArray",QJsonArray());
                    }
                    QJsonArray ary=mJsonObject.value("PickedPosArray").toArray();
                    if(ary.size()==2)
                    {
                        return;
                    }
                    ...
                    double raspt[2]{pickedpt[0]*spacing[0],pickedpt[1]*spacing[1]};
                    imageData->GetCenter(center);
                    double distance=std::sqrt((center[0]-raspt[0])*(center[0]-raspt[0])+(center[1]-raspt[1])*(center[1]-raspt[1]));
                    mJsonObject["PickedPosArray"]=ary;
                    mJsonObject[text]=QJsonValue(distance);
                    doc.setObject(mJsonObject);
                    QByteArray bt=doc.toJson();
                    file.resize(0);
                    file.write(bt);
    ...
    //设置右键菜单策略为上下文菜单
    pTarget->setContextMenuPolicy(Qt::ActionsContextMenu);
}
...
//    connect(ui->comboBox_vetebra,&CheckComboBox::signal_checkStateChanged,this,&RegisterFineAdjustDialog::checkChanged);
    ...
    connect(ui->checkBox_showRgb,&QRadioButton::clicked,this,&RegisterFineAdjustDialog::showRgb);
    connect(ui->checkBox_pickPoint,&QRadioButton::clicked,this,[&]
    {
        //添加选点时禁用窗宽窗位调节功能
//        bool isChecked=ui->checkBox_pickPoint->isChecked();
//        if(isChecked)
//        {
//            styleImageFront->setAdjustEnable(0);
//            styleImageSide->setAdjustEnable(0);
//        }
//        else
//        {
//            styleImageFront->setAdjustEnable(1);
//            styleImageSide->setAdjustEnable(1);
//        }
    ...
    connect(ui->pushButton_clearPoints,&QPushButton::clicked,this,&RegisterFineAdjustDialog::clearPoints);
...
void RegisterFineAdjustDialog::adjust()
{
    ...
    siddonGPUFront->SetImg2dParameter(spacingFront, imageSizeFront, mask);
    ...
    RawImageType::PointType  imOrigin = m_rawImage->GetOrigin();
    RawImageType::SpacingType imRes = m_rawImage->GetSpacing();

    typedef RawImageType::RegionType     InputImageRegionType;
    typedef InputImageRegionType::SizeType InputImageSizeType;

    InputImageRegionType imRegion = m_rawImage->GetBufferedRegion();
    ...
    {
        for (int j = 0; j < 4; ++j)
            transformMatrixFront[i*4+j] = mt[i][j];
    }
    ...
    ShiftScale->SetInputData(ret);
    ShiftScale->SetOutputScalarTypeToUnsignedShort();
    ShiftScale->SetShift(0);
    ShiftScale->SetScale(1./(ret->GetScalarRange()[1]-ret->GetScalarRange()[0])*4095);
    ShiftScale->Update();
    short* buffer = (short *)ShiftScale->GetOutput()->GetScalarPointer();
    for (int i = 0; i < ImageSize; i++)
    {
        buffer[i] = 4095 - buffer[i];
    }
    ShiftScale->GetOutput()->Modified();
    ...
    bool isRgb=ui->checkBox_showRgb->isChecked();
    showImages(ShiftScale->GetOutput(),nullptr);
}
...
void RegisterFineAdjustDialog::adjustByOpacity()
{
    ...
    double originOpacity=ui->horizontalSlider_Opacity->value()/100.;
    bool isRgb=ui->checkBox_showRgb->isChecked();
    typedef itk::FlipImageFilter<itk::Image<float, 2>> FlipfilterType;
    typedef FlipfilterType::FlipAxesArrayType FlipAxesArrayType;
    FlipfilterType::Pointer filterFront=FlipfilterType::New();
    FlipAxesArrayType flipArray;
    flipArray[0]=0;
    flipArray[1]=1;
    filterFront->SetFlipAxes(flipArray);
    filterFront->SetInput(processedImageFront);
    filterFront->Update();
    FlipfilterType::Pointer filterSide=FlipfilterType::New();
    filterSide->SetFlipAxes(flipArray);
    filterSide->SetInput(processedImageSide);
    filterSide->Update();
    showImagesVtk(filterFront->GetOutput(),filterSide->GetOutput(),originOpacity,isRgb);
}
...
void RegisterFineAdjustDialog::clearPoints()
{
    ...
    int windowSide=mActorMergedSide->GetProperty()->GetColorWindow();
    if(styleImageFront)
        styleImageFront->clearPoints();
    if(styleImageSide)
        styleImageSide->clearPoints();
    adjust();
    mActorMergedFront->GetProperty()->SetColorLevel(levelFront);
    mActorMergedFront->GetProperty()->SetColorWindow(windowFront);
    mActorMergedSide->GetProperty()->SetColorLevel(levelSide);
