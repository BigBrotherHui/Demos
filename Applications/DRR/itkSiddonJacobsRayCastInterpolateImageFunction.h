...
namespace itk
{
	...
	public:
		...
		void PrepareMemory4Cuda();
		void getTransformMatrix(float* M);
		void DRRCudaRun(float* rst1);
		virtual void
			Initialize();
		...
		itkSetMacro(bObl, bool);
		itkGetMacro(bObl, bool);

		/** Set and get the Threshold */
		itkSetMacro(Threshold, double);
		itkGetMacro(Threshold, double);

		itkSetMacro(FocalDistance, float);
		itkGetMacro(FocalDistance, float);
		...
		inline void
			SetImgSize(Size<3> _imgsize)
		{
			m_ImgSize[0] = _imgsize[0];
			m_ImgSize[1] = _imgsize[1];
		}
		inline void
			SetPixelspacing(itk::Vector<double, 3> _Pixelspacing)
		{
			m_Pixelspacing[0] = _Pixelspacing[0];
			m_Pixelspacing[1] = _Pixelspacing[1];
		}
		...
		inline void
			SetCenterOff(std::vector<float> pt)
		{
			for (int i = 0; i < 3; ++i)
			{
				m_fCenterOff[i] = pt[i];
