...
namespace itk
{
	PowellOptimizerTest::PowellOptimizerTest()
	{
	...
	PowellOptimizerTest::~PowellOptimizerTest() = default;

	void
		PowellOptimizerTest::SetLine(const PowellOptimizerTest::ParametersType & origin, const vnl_vector<double> & direction)
	{
	...
	double
		PowellOptimizerTest::GetLineValue(double x) const
	{
		PowellOptimizerTest::ParametersType tempCoord(m_SpaceDimension);

		return this->GetLineValue(x, tempCoord);
	}

	double
		PowellOptimizerTest::GetLineValue(double x, ParametersType & tempCoord) const
	{
		for (unsigned int i = 0; i < m_SpaceDimension; i++)
		{
	...
	void
		PowellOptimizerTest::SetCurrentLinePoint(double x, double fx)
	{
		for (unsigned int i = 0; i < m_SpaceDimension; i++)
		{
	...
	void
		PowellOptimizerTest::Swap(double * a, double * b) const
	{
		double tf;
	...
	void
		PowellOptimizerTest::Shift(double * a, double * b, double * c, double d) const
	{
		*a = *b;
		*b = *c;
		*c = d;
	...
	//
	void
		PowellOptimizerTest::LineBracket(double * x1, double * x2, double * x3, double * f1, double * f2, double * f3)
	{
		PowellOptimizerTest::ParametersType tempCoord(m_SpaceDimension);
	...
	void
		PowellOptimizerTest::LineBracket(double *         x1,
			double *         x2,
			double *         x3,
			double *         f1,
			double *         f2,
	...
	void
		PowellOptimizerTest::BracketedLineOptimize(double   ax,
			double   bx,
			double   cx,
			double   fa,
	...
	void
		PowellOptimizerTest::StartOptimization()
	{
		...
		PowellOptimizerTest::ParametersType p(m_SpaceDimension);
		PowellOptimizerTest::ParametersType pt(m_SpaceDimension);
		PowellOptimizerTest::ParametersType ptt(m_SpaceDimension);
	 ...
	 */
	const std::string
		PowellOptimizerTest::GetStopConditionDescription() const
	{
	 ...
	 */
	void
		PowellOptimizerTest::PrintSelf(std::ostream & os, Indent indent) const
	{
